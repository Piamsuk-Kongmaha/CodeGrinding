...
int32_t main() {
    ...
    for (int i = N - 1; i >= 0; --i) {
        ...
        if (i < N - 1) {
            int right_diff = abs(Heat[i] - Heat[i + 1]);
    ...
    vector<unsigned long long> prefix_left_to_right(N, 0);
    for (int i = 1; i < N; ++i) {
        prefix_left_to_right[i] = prefix_left_to_right[i - 1] + diff_left_to_right[i - 1];
    }
 
    vector<unsigned long long> prefix_right_to_left(N, 0);
    for (int i = N - 2; i >= 0; --i) {
        prefix_right_to_left[i] = prefix_right_to_left[i + 1] + diff_right_to_left[i + 1];
    }
